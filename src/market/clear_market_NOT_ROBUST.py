...
def clear_market(*,
    ...
    while i < len(supply) and j < len(demand):
        ...
        if supply.loc[i, "remaining"] <= EPS:
            supply.loc[i, "remaining"] = 0.0
            i += 1
        if demand.loc[j, "remaining"] <= EPS:
    ...
    #     )
    if cleared_quantity == 0:
        df_empty = supply.copy()
        df_empty["cleared_quantity"] = 0.0
        df_empty["clearing_price"] = np.nan
        df_empty["market_incomes"] = 0.0
        return None, 0.0, df_empty.set_index("technology")
